...
let BtnImportFromGoogleDom = document.querySelector(
  `#btn-import-from-google-app`
)
...
Authenticator.prototype.newDom = function () {
  ...
  if (this.email) {
    newDom.querySelector(`.atoken`).textContent =
      '(' + this.issuer + ') ' + this.email
    ...
    protobuf.load('.googleAuth', function (err, root) {
      ...
      result.otpParameters.map(account => {
        var authenticator = new Authenticator(
          toBase32(Buffer.from(account.secret, 'base64')),
          account.name,
          account.issuer
        )
...
window.onload = function () {
  ...
  document
    .querySelector(`.import_from_google_modal_confirm`)
    .addEventListener('click', doImportGoogleAuthenticator)
